# Inbox triage and reply

Classify messages, draft replies, identify commitments, and request approval before any send.
...
| Sample contract | Ready | `examples/samples/personal/inbox-triage-and-reply.sample.json` covers required inputs and expected outputs. |
| Run evidence | Open | A completed run needs a runtime wired to the live inbox and an approval surface; a standalone run would fabricate that state, so none ships. |
