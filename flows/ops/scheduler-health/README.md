...
Inspect recurring automations, cron jobs, reminders, and monitors for failures or drift, with remediation approved before changes.
...
| Sample contract | Ready | `examples/samples/ops/scheduler-health.sample.json` covers required inputs and expected outputs. |
| Run evidence | Open | A completed run needs a runtime wired to the live schedulers; a standalone run would fabricate that state, so none ships. |
