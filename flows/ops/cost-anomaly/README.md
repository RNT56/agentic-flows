...
Diagnose unexpected cloud, API, or compute spend from source billing data and recommend an owned, risk-assessed action.
...
| Sample contract | Ready | `examples/samples/ops/cost-anomaly.sample.json` covers required inputs and expected outputs. |
| Run evidence | Open | A completed run needs a runtime wired to the billing system; a standalone run would fabricate that state, so none ships. |
