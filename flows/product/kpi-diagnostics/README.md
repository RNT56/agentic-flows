...
Explain why a metric changed and what to do next, separating drivers from speculation with caveats.
...
| Sample contract | Ready | `examples/samples/product/kpi-diagnostics.sample.json` covers required inputs and expected outputs. |
| Run evidence | Open | A completed run needs a runtime wired to the real metrics source; a standalone run would fabricate that state, so none ships. |
