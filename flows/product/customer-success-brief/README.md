...
Prepare a customer or account briefing from tickets, usage, docs, and roadmap, scoping sensitive data and linking sources.
...
| Sample contract | Ready | `examples/samples/product/customer-success-brief.sample.json` covers required inputs and expected outputs. |
| Run evidence | Open | A completed run needs a runtime wired to the real tickets, usage, and roadmap; a standalone run would fabricate that state, so none ships. |
