...
Analyze an experiment and decide ship, iterate, or stop, reviewing sample size and guardrails so the decision follows the evidence.
...
| Sample contract | Ready | `examples/samples/product/experiment-readout.sample.json` covers required inputs and expected outputs. |
| Run evidence | Open | A completed run needs a runtime wired to the real experiment results; a standalone run would fabricate that state, so none ships. |
