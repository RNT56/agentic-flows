...
- Completed the deferred backlog as 19 contract-first flows (personal daily-command-center / inbox-triage-and-reply / cross-channel-briefing; engineering merge-readiness / release-train; ops cost-anomaly / rollback / infrastructure-drift / backup-restore-drill / scheduler-health; product kpi-diagnostics / experiment-readout / churn-risk-review / customer-success-brief / pricing-or-packaging-analysis / release-positioning; security secret-leak-response; research competitor-watch; docs public-announcement). Each ships a validating `flow.yaml`, a README maturity rubric, and a sample, with a `judgment` decision gate (reviewer-identity) and a source gate declaring `external-production` evidence — and deliberately **no completed run bundle**, because a standalone run would fabricate the live external state the flow depends on. A new test enforces that invariant: an `external-production` flow may not ship a completed run bundle. (The deferrals that were honesty problems solvable with a sandbox or bounded scope shipped run-evidenced earlier as `ops.ephemeral-preview-deploy`, `engineering.browser-matrix-check`, and `program.feature-to-release`.)
...
- Corrected `supported_cores` assignments across the v1.1 and contract-first flows to match the catalog convention that operator control belongs to ThinClaw: every leaf flow with an `approval` node or `human.review` now lists `thinclaw` (and declares `human.review`), and the composition programs use their `flow_ref` children's intersection rather than an over-broad core list. ThinClaw (approvals/operator control), NilCore (sandboxed execution), and CrustCore (verifier-owned completion) are distinct independent consumers and are no longer conflated.
